...
class repositoryModel
{
  ...
  public static function getAllPaginated(string $table, int $pageNumber, int $pageSize, array $data, string $field = '', string $value = '', string $ascending)
  {
    self::$conn = db::connect();
    ...
    $conditions = array_filter($data, fn($val) => isset ($val) && $val !== '');
    $params = implode(' AND ', array_map(fn($key) => "$key = :$key", array_keys($conditions)));
  ...
  public static function getAllPaginatedByLike(string $table, int $pageNumber, int $pageSize, array $data, string $field = '', string $value = '', string $ascending)
  {
    self::$conn = db::connect();
  ...
  public static function getAllDataByColumn(string $table, string $field, $value)
  {
    ...
    if (is_object(self::$conn)) {
      ...
      $sql = 'SELECT * FROM ' . db::get('DB_NAME') . '.' . $table . ' WHERE ' . $field . '=:value';
      $stmt = self::$conn->prepare($sql);
